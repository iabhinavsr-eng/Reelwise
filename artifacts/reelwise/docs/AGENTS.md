# Reelwise Design System design system
...
workspace package (`@workspace/reelwise`): other artifacts depend
...
- `docs/references/` — retained onboarding HTML used to derive the initial
  visual language; see its manifest for source and scope.
- `SKILL.md` — source-derived visual and writing guidance for future work.
...
`@workspace/reelwise/components/...`. Never use a `@/` alias inside
...
### Reelwise visual guidance

- Use near-black ink for primary actions. Reserve Reelwise green for progress,
  focus, selected states, and concise emphasis.
- Use soft mint for helpful context and suggested states, distinct from primary
  actions. Keep fields and cards comfortably rounded and borders quiet.
- Preserve the mobile-first hierarchy of a compact step label, clear headline,
  and short supporting copy.
- Keep onboarding language reassuring and direct; explain what a setting changes
  before asking someone to choose it.
- The reference uses a native system-font stack. The web preview uses Inter as a
  portable stand-in because no licensed font file was supplied.
- No separate logo file was supplied; retain the source's text wordmark instead
  of inventing a mark.
- The source provides light mode only. Dark mode is derived, not a direct match.
...
for `@workspace/reelwise` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/reelwise` as a `workspace:*` dependency,
...
- Building or styling web UI: `artifacts/reelwise/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/reelwise/docs/consuming-expo.md`
- Building or styling a slide deck: `artifacts/reelwise/docs/consuming-slides.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/reelwise/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/reelwise/docs/migrating-expo.md`
