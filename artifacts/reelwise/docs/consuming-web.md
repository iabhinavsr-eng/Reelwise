# Consuming Reelwise Design System in web apps

Read `artifacts/reelwise/docs/AGENTS.md` first. This guide covers
...
`artifacts/reelwise/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/reelwise/styles.css";
```
...
`node_modules/@workspace/reelwise/src/components` to `content`.
...
import { cn } from "@workspace/reelwise/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/reelwise/hooks/use-toast";
```
...
`@workspace/reelwise/components/ui/button`. Run the app's typecheck
