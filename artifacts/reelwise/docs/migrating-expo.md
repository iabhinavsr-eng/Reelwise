# Migrating Expo UI to Reelwise Design System

Read `artifacts/reelwise/docs/AGENTS.md` and
`artifacts/reelwise/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/reelwise/lib/native-theme` and
  `@workspace/reelwise/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/reelwise/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/reelwise` and retains only
